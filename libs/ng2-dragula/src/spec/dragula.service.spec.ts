...
describe('DragulaService', () => {
  ...
  //   let called: T;
  //   const fn = jest.fn();
  //   obs.pipe(take(n > 0 ? n : 1)).subscribe(ev => {
  //     fn(ev);
  //     called = ev;
  //     trigger();
  //     // subs.unsubscribe();
  //     expect(fn(td.matchers.isA(Object).mock.calls.length)).toBe(n);
  //   });
  //   return called;
  // };
  ...
  xit('should fire drag when drake does', () => {
  ...
  xit('should not fire drag for an irrelevant drag type', () => {
    const ul = buildList('ul', ['li']);
    const li = ul.children[0];
  ...
  xit('should start listening to drake events when drake without models passed to add', done => {
    ...
    //   mock.emit(EventTypes.Drag, li, ul);
    // });
    // service.drag("NOMODELS").subscribe(ev => {
    //   console.log('ev', ev);
    //   fn(ev as any);
    //   called = ev;
    //   mock.emit(EventTypes.Drag, li, ul);
    ...
    // });
    service.drag("NOMODELS").subscribe(ev => {
      console.log('ev there there there there');
      called = true;
  ...
  it('#getObservableValue should return value from observable',
      ...
      service.drag("NOMODELS").subscribe(value => {
